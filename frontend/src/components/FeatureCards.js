...
const FeatureCards = () => {
    return (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-6xl mx-auto">
      {features.map((feature, index) => (<framer_motion_1.motion.div key={feature.title} initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{
                duration: 0.6,
                delay: 0.6 + index * 0.1,
                type: "spring",
                stiffness: 100
          ...
          <div className="relative glass-effect rounded-2xl p-6 h-full border border-white/10 group-hover:border-white/20 transition-all duration-300">
            ...
            {/* Hover indicator */}
            <framer_motion_1.motion.div className={`absolute bottom-0 left-0 h-1 bg-gradient-to-r ${feature.gradient} rounded-b-2xl`} initial={{ width: 0 }} whileHover={{ width: '100%' }} transition={{ duration: 0.3 }}/>
          </div>
